...
const AddReservation = (props) => {
  ...
  const add = (price) => {
    props.addReservation(props.route.params.id,props.tenant.id,props.route.params.landlord,capacity,in_date, out_date, price);
    setIn_date('');
    setOut_date('');
  ...
  const daysCount = () => {
    if(in_date && out_date && capacity){
      ...
      {
        Alert.alert("Error", "La date de fin doit etre superieur à la date de départ");
      }
      else if(Difference_In_Days>0)
      {
        Alert.alert("Confirmation", "Total price : "+(props.route.params.price*Difference_In_Days).toString()+" $",
        [{text:"Yes",onPress:()=>{add(props.route.params.price*Difference_In_Days)}},{text:"Return"}]);
      }
    }
    else{
      ...
      >
        ...
        <ScrollView style={styles.container}>
          ...
          <Input leftIcon={{ type: 'font-awesome', name: 'calendar', color: '#517fa4' }}
            type="date" id="start" name="trip-start" min="2020-01-01" max="2025-12-31" placeholder='AAAA-MM-JJ'
            onChangeText={date => {setIn_date(date)}} />
          <Text for="start">Date de Fin :</Text>
          <Input leftIcon={{ type: 'font-awesome', name: 'calendar', color: '#517fa4' }}
            type="date" id="start" name="trip-start" placeholder='AAAA-MM-JJ' min="2020-01-01" max="2025-12-31"
