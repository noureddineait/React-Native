  ...
  const editProfilHandler = () => {
    if (username && mail_address && password && first_name && last_name && birth_date && gender) {
      const foundAdress = props.landlords.some(
        el => el.mail_address === mail_address && el.id != props.landlord.id,
      );
      const foundUser = props.landlords.some(
        el => el.username === username && el.id != props.landlord.id,
      );

      if (!(foundUser || foundAdress)) {
        props.updateLandlord(
          props.landlord.id,
          first_name,
          last_name,
          mail_address,
